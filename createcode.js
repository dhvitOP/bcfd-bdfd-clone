...
module.exports = {
  ...
  if(!cmdname)
  {
    ...
    let chek = await db.fetch(botdata.stoken, `templates_${cmdname}`);
    
   if(chek)
   {
     return message.channel.send("template exist with that code");
   }
   let bots = await botsdata.find();
   bots.forEach((bot) => {
 db.set( `templates_${cmdname}`, true);
   })


  message.channel.send("Created Code");
}
}
